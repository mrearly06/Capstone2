    ...
    if (isset($_POST['approve'])) {
        ...
        try {
            ...
            if ($itemReview) {
                ...
                // Fetch item reviews for a given inventoryReqID
                $getReviewsQuery = "SELECT * FROM item_review WHERE inventoryReqID = ?";
                $stmt = $conn->prepare($getReviewsQuery);
                $stmt->bind_param("s", $inventoryReqID);  // Assuming $inventoryReqID is available
                $stmt->execute();
                $reviewsResult = $stmt->get_result();

                try {
                    // Your code to fetch item reviews and insert them into the `item` table
                
                    if ($reviewsResult->num_rows > 0) {
                        // Loop through all the item reviews and insert them into the `item` table
                        while ($itemReview = $reviewsResult->fetch_assoc()) {
                    ...
                    // On successful insert, set the session variable and redirect
